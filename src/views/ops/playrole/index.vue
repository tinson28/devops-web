    ...
    <el-row :gutter="28">
      <el-col :span="span">
        ...
        <el-table v-loading="loading" ref="tables" :data="data" highlight-current-row size="small" border style="width: 100%;" >
          ...
          <el-table-column v-if="table_show" prop="role_desc" label="角色描述"/>
          <el-table-column v-if="table_show" prop="role_time" label="创建时间">
            <template slot-scope="scope">
                <i class="el-icon-time" v-show="scope.row.role_time"></i>
                <span>{{parseTime(scope.row.role_time)}}</span>
            </template>
          </el-table-column>
          <el-table-column label="操作" align="center" >
            <template slot-scope="scope">
              <!--<edit v-if="checkPermission(['admin','inventory_all','inventory_edit'])" :data="scope.row" :sup_this="sup_this"/>-->
              <el-button v-if="checkPermission(['admin','playrole_all','playrole_list'])" size="mini" type="primary" @click="toDetail(scope.row.id,scope.row.role_name)">详情</el-button>
              <el-popover
                v-if="checkPermission(['admin','inventory_all','inventory_delete'])"
                :ref="scope.row.id"
                placement="top"
                width="180">
                <p>确定删除本条数据吗？</p>
      ...
      <el-col :span="12">
        <el-card v-if="show && checkPermission(['admin','inventory_all','inventory_edit'])" class="box-card">
...
import { del, save, retrieve } from '@/api/ansiblerole'
...
export default {
  ...
  methods: {
    ...
    // 详情
    toDetail(id,role_name) {
      this.$router.push({
        path: 'playrole/detail',
        query: {id: id, isAdd: false ,name:role_name}
      })
    },
    cancel() {
      this.span = 24
