    ...
    <el-row :gutter="28">
      <el-col :span="span">
        ...
        <el-table v-loading="loading" ref="tables" :data="data" highlight-current-row size="small" border style="width: 100%;" >
          ...
          <el-table-column prop="ans_group_name" label="主机组名称" width="200px"/>
          <el-table-column v-if="table_show" prop="ans_group_vars" label="主机组变量" width="200px"/>
          <el-table-column v-if="table_show" prop="ans_group_memo" label="主机组描述" />
          <el-table-column v-if="table_show" label="创建时间" >
            <template slot-scope="scope">
                <i class="el-icon-time" v-show="scope.row.ans_group_datetime"></i>
                <span>{{parseTime(scope.row.ans_group_datetime)}}</span>
            </template>
          </el-table-column>
...
export default {
  components: { eHeader, edit, relate },
  mixins: [initData],
  data() {
    return {
      row_data: null,
